...
-- (drizzle/0043-0047) and NOT the rest of the catalog. Older migrations carry
-- definer functions that do not yet comply; they are the concurrent security
-- pipeline's drizzle/0048 (item B-M2, widened). When 0048 lands it WIDENS THIS
-- SAME ASSERTION — drops the name array so it becomes catalog-wide — rather
-- than adding a second, parallel one, so there is never a moment when two
-- assertions could disagree about what "compliant" means.
begin;
