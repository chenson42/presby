...
/**
 * Orchestration tests for `/o/<slug>/admin/reports`'s page.tsx — product-IA
 * scaffold, docs/work-log/2026-08-27-product-ia-scaffold.md (Phase 3 §3/§9,
 * DECISION-117). Mirrors `admin/committees/page.test.tsx`'s
 * flag-before-org-type ordering contract. `reports` is a presbytery-scoped
 * placeholder tile (`category: "administer"`, unlike `insights`).
 */
...
describe("ReportsPage — the shared four-way miss response", () => {
  it("redirects to /signin with a callbackUrl back to admin/reports when unauthenticated", async () => {
    ...
    await expect(ReportsPage({ params: makeParams() })).rejects.toThrow(
      "REDIRECT:/signin?callbackUrl=%2Fo%2Fnorthern-reach%2Fadmin%2Freports",
    );
  ...
  it("calls notFound() for a slug with no organization", async () => {
    ...
    await expect(ReportsPage({ params: makeParams() })).rejects.toThrow("NOT_FOUND");
  });
  ...
  it("renders the shared access-denied copy for a forbidden org relationship", async () => {
    ...
    const el = await ReportsPage({ params: makeParams("other-presbytery") });
    render(el);
  ...
  it("renders the shared ended-relationship copy for an ended relationship", async () => {
    ...
    const el = await ReportsPage({ params: makeParams("fernwood") });
    render(el);
...
describe("ReportsPage — the flag-before-org-type ordering contract", () => {
  ...
  it("flag off renders PlaceholderFlagOff, checked with org_portal.reports", async () => {
    cachedAuth.mockResolvedValue({ user: { id: "u1" } });
    resolveOrgContext.mockResolvedValue(OK_RESOLVED_PRESBYTERY);
    isFlagEnabled.mockResolvedValue(false);

    const el = await ReportsPage({ params: makeParams() });
    render(el);
  ...
  it("flag off wins over org type — a congregation with the flag off still sees PlaceholderFlagOff, not PlaceholderNotAvailable", async () => {
    ...
    const el = await ReportsPage({ params: makeParams("alder-creek") });
    render(el);
  ...
  it("flag on + correct org type (presbytery) renders ComingSoon", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Per-Capita, SASR & Imports" }),
    ).toBeTruthy();
    expect(
