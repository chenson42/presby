...
/**
 * Orchestration tests for `/o/<slug>/admin/communications`'s page.tsx —
 * product-IA scaffold, docs/work-log/2026-08-27-product-ia-scaffold.md
 * (Phase 3 §3/§9, DECISION-117). Mirrors `admin/giving/page.test.tsx`'s
 * assertion style. `communications` is a universal tile (no `orgTypeScope`).
 */
...
describe("CommunicationsPage — the shared four-way miss response", () => {
  it("redirects to /signin with a callbackUrl back to admin/communications when unauthenticated", async () => {
    cachedAuth.mockResolvedValue(null);

    await expect(CommunicationsPage({ params: makeParams() })).rejects.toThrow(
      "REDIRECT:/signin?callbackUrl=%2Fo%2Falder-creek%2Fadmin%2Fcommunications",
    );
  ...
  it("renders the shared access-denied copy for a forbidden org relationship", async () => {
    ...
    const el = await CommunicationsPage({ params: makeParams("other-church") });
    render(el);
  ...
  it("renders the shared ended-relationship copy for an ended relationship", async () => {
    ...
    const el = await CommunicationsPage({ params: makeParams("fernwood") });
    render(el);
...
describe("CommunicationsPage — auth gate ordering", () => {
  it("calls assertOrgAccess before checking the flag", async () => {
    cachedAuth.mockResolvedValue({ user: { id: "u1" } });
    resolveOrgContext.mockResolvedValue(OK_RESOLVED);
    isFlagEnabled.mockResolvedValue(false);

    await CommunicationsPage({ params: makeParams() });

    expect(assertOrgAccess).toHaveBeenCalledWith("person-1", "org-1");
  });
});

describe("CommunicationsPage — flag states", () => {
  it("flag off renders PlaceholderFlagOff naming the org, checked with org_portal.communications", async () => {
    cachedAuth.mockResolvedValue({ user: { id: "u1" } });
    resolveOrgContext.mockResolvedValue(OK_RESOLVED);
    isFlagEnabled.mockResolvedValue(false);

    const el = await CommunicationsPage({ params: makeParams() });
    render(el);

    expect(isFlagEnabled).toHaveBeenCalledWith("org_portal.communications");
    expect(
