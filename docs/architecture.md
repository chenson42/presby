...
*A first read for engineers joining the project. This document explains what we're building and why, and the handful of decisions that shape everything else. It intentionally stays high-level — each section links to the document that has the actual detail, rules, and rationale.*

*Last written: 2026-08-20. The name `presby` is a placeholder — see [`docs/STATE.md`](STATE.md) for naming status; it doesn't affect anything below.*

---

## 1. What this is, and why it exists

**presby is a multitenant platform for Presbyterian congregations, presbyteries, and synods** — the three levels of a PC(USA) governing structure. One church management system serves membership rolls, officer records, a member directory, giving, and events; one set of tools serves council operations (the presbytery/synod layer above a congregation); every tenant gets a public website; and a support-ticket loop, worked partly by an AI pairing with a human operator, is how the platform grows and gets fixed.
...
4. **Authority flows up by publication, never down by inheritance.** A presbytery sees what a congregation is required to publish to it (the statistical report) and nothing else by default. The only two exceptions — an administrative commission, and a session-granted delegation — are both explicit, time-boxed, and minuted, not ambient access.
...
Next.js 16 (App Router) and React 19, TypeScript throughout in strict mode. Drizzle ORM against Neon Postgres, deliberately over a WebSocket connection pool rather than Neon's simpler HTTP driver — the isolation model in section 3 depends on a transaction-scoped session variable, and the HTTP driver has no notion of a transaction, so the isolation model dictated the driver, not the other way around. NextAuth for authentication, Tailwind and Radix for UI primitives, deployed to Vercel.
