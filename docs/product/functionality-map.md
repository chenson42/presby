...
- **presby: NOT built** — ledger/giving, events, worship, check-in, sites not designed (site architecture locked, pipeline in design — `docs/work-log/2026-08-20-public-sites.md`). Within schema/authorization: roll UI (read path complete, unsurfaced), officer-term management, new-role/new-permission creation, the cross-org commission/delegation UI, and a tenant-facing audit reader (DECISION-067, deferred pending a FORCE-RLS `audit_events` projection) remain unbuilt.
