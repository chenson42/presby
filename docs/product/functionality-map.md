...
- **presby: org portal** — `/o/<slug>` tenant-facing pages beyond the landing stub: directory (`/o/<slug>/directory`, `directory.view` permission, privacy-filtered, gated `org_portal.directory`) and role administration (`/o/<slug>/admin/roles`, `role_grants.manage` permission, gated `org_portal.roles`, both flags seeded off) — grant/revoke/view *existing* `role_grants` rows against *existing* `app_roles`, creates neither. First tenant-facing capability a congregation's own staff — not a platform admin — can act on. `src/lib/directory.ts`, `src/lib/role-grants.ts`, `src/app/(org)/o/[slug]/{directory,admin/roles}/`.
...
- **presby: NOT built** — ledger/giving, events, worship, check-in, sites, tickets not designed. Within schema/authorization: roll UI (read path complete, unsurfaced), officer-term management, new-role/new-permission creation, the cross-org commission/delegation UI, and a tenant-facing audit reader (DECISION-067, deferred pending a FORCE-RLS `audit_events` projection) remain unbuilt.
