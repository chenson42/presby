...
- [ ] **P0.5 — design foundation + brand architecture, slices b/c(admin)/e** — Phases 1–2 **re-run and complete** for the remaining scope (2026-08-19): the ramp generator (b), brand storage + admin-only emission (c), type pairings (e). Three findings resolved as DECISION-054/055/056 — an unpoliced `accent` token patched into the contract, slice c builds DECISION-030's storage adapter itself (logo as first consumer), and slice c's public/anonymous read path is explicitly deferred to P3 rather than built against infrastructure that doesn't exist. Phase 3 (tech-lead) in progress. Slice d still depends on P1's tenant permission catalog. — `docs/work-log/2026-08-19-brand-foundation.md`
...
- [ ] **`npm run db:generate` is broken repo-wide, a different failure mode from the `db:push` drift above.** `drizzle/meta/0009`–`0012_snapshot.json` are byte-identical copies of `0008`'s `id`/`prevId` — a real collision, not phantom drift — so drizzle-kit refuses to compute any diff at all. Migrations `0013`–`0016` have all been hand-authored with a manually-registered `_journal.json` entry as a workaround; either repair the snapshot chain or make the hand-authored pattern the documented house style in `CLAUDE.md`/`docs/schema-design.md` — `docs/work-log/2026-08-19-brand-foundation.md` Phase 4 commit `c1`
