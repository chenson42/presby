...
Architectural and implementation decisions for PresbyPortal (presby). Newest first. Each decision is numbered; the number does not change once assigned.
...
DECISION-048 ruled that an organization's brand may not retune corner roundness — `--radius` is `platform` in the closed partition (`src/lib/brand/contract.ts`) — and that ruling stands unchanged (reaffirmed by DECISION-142). Its descriptive aside about the platform's own value was already stale before this pipeline (the stylesheet read `0.5rem` going in) and is now doubly so; a comment in `globals.css` can carry the current fact, but only this log can carry the auditable statement that the value changed and how it was verified. The value moved from `0.5rem` to `0.625rem` by an operator decision after Track B shipped, kept out of Track B so the typeface diff stayed legible. The derived scale is unchanged in form (`calc(var(--radius) ± Npx)`), so `sm`/`md`/`lg`/`xl` now compute to 6/8/10/14px; `full` and the un-remapped sizes do not move; public sites keep their own `--site-radius` and are unaffected.

**How a platform-wide token change is verified, stated once:** not by the visual-parity harness's pass/fail diff — with `maxDiffPixels: 0` every route fails by design — but by re-baselining before and after from production builds and reviewing the screenshots as before/after pairs. This pipeline also established the limit of that harness: `visual:baseline` rewrites a baseline only when pixelmatch reports a mismatch at Playwright's default `threshold: 0.2` (per-pixel YIQ cutoff 1409), so low-contrast geometry changes are certified sub-threshold rather than captured (13 of 80 here). That is adequate evidence for a 2px corner change and would not be for a change with real visual stakes such as colour contrast; the harness header's stronger claim is tracked for correction in `docs/TODO.md`.

---

**DECISION-144: FPCW's `/signin` stays un-branded — `ui.branded_signin` remains OFF for FPCW as an operator decision — and fpcw visual parity is signed off by a human side-by-side, not by the screenshot harness.** (2026-09-25, architect Phase 2, `docs/work-log/2026-09-25-brand-visual-parity.md`.)

DECISION-047's designed refusal is preserved, with the narrowing that the relevant mechanism is now DECISION-094's flagged, page-scoped exception rather than a blanket rule: the flag exists, is seeded off (`scripts/seed.ts:375-388`), and would brand FPCW's sign-in the day `site-fpcw` goes live and someone flips it. Recorded as a decision so the omission is deliberate rather than inherited. The brandable surface count is unchanged — two layouts plus that one page, `check-brand-scope.mjs`'s `EMITTERS` untouched. Separately: `e2e/visual-parity.spec.ts` self-compares two presby runs at `maxDiffPixels: 0` and therefore proves only that nothing else moved; it is regression-only and a PASS from it is not evidence of parity. Parity is the operator's own judgment against the local fpcw-directory instance, and Phase 3 names them as the signer. CLAUDE.md's "The Brand Is a Cascade Override" section gains one line recording DECISION-094's exception, which it currently omits.

---

**DECISION-143: `--font-heading`/`--font-body` become load-bearing through static per-pairing CSS classes and an `@layer base` consumer with an `inherit` fallback — not through a font role in `BRAND_ROLES` — and no fifth type pairing is added.** (2026-09-25, architect Phase 2, `docs/work-log/2026-09-25-brand-visual-parity.md`.)

`ROLE_TO_TOKEN`'s codomain is `PlatformTokenName` precisely so every role has a platform default to measure against (`contract.ts:319-340`); `nonColour` tokens are excluded, and `generateBrandTokens()` is a pure function of a colour seed. A font role would force that type open, void the generator's totality check and the 294-seed grid for two roles, and hand the emitter data it cannot produce. Instead: `fonts.ts` additionally exposes `next/font`'s `.variable` class (today `RESOLVED_PAIRINGS` stores only variable *name strings*, which is why `--font-heading-classic` is defined nowhere in the document — `docs/TODO.md:218`); `globals.css` gains four static `.pairing-<key>` rules mapping the generic properties to the per-pairing ones, plus `h1`–`h6`/`body` consumers using `var(--font-heading, inherit)` so every un-branded surface renders byte-identically; the two brandable layouts and `(auth)/signin/page.tsx` apply the classes. `additive: true` stays accurate and the closure test is untouched (its parser reads only `:root`/`.dark`, `contract.test.ts:193-236`). On the pairing: Geist **is** Google-Fonts-hosted and `next/font/google`-resolvable — the parity target itself imports it (`fpcw-directory/src/app/layout.tsx:2`) and Next's own `font-data.json` carries it — so the constraint Phase 1 assumed does not exist; the pairing is refused on merit instead. fpcw-directory's Geist is `create-next-app`'s scaffold default, not FPCW's identity; FPCW's own live site is Montserrat/Open Sans, already curated as `contemporary` for this congregation by DECISION-093. No fifth entry, no `organization_brands_type_pairing_allowed` widening, no `drizzle/` touch. Had one been needed it would go through a narrow database-admin-owned schema work-log, not a Rule 16 carve-out: hand-written `presby_*` migrations take a `drizzle/meta/_journal.json` entry, and that file plus the pre-assigned migration number are orchestrator-only under Workflow Rule 16.

---

**DECISION-142: Track B's brand-vocabulary widening is withdrawn — `--card`/`--popover`/`--muted`/`--secondary`/`--border`/`--input` stay `platform`, `--radius` stays `platform` per DECISION-048, and `BRAND_TOKEN_VERSION` does not move.** (2026-09-25, architect Phase 2, `docs/work-log/2026-09-25-brand-visual-parity.md`; confirmed by the operator 2026-09-25 — see the work-log.)

The widening's stated justification ("fpcw brands these and presby's contract cannot express it") is false on direct inspection: `~/git/fpcw-directory/src/app/globals.css:61-128` carries unmodified stock shadcn achromatic values for all six tokens in both schemes, and its `#60a7a1` teal exists only as a decorative `@theme` utility scale (`--color-brand-50…900`, `:12-22`) used ad hoc across 51 files with no relationship to the semantic token system. Only the operator's general "flexibility" approval remained, and it does not carry any of the six: `--card` needs four new `LEGAL_PAIRS` and breaks D4's structural ring-offset closure (the offset is painted `ring-offset-background` against a now-divergent `--card`); `--popover` additionally requires an adjacent-surface distinguishability floor that WCAG does not supply and `contract.ts:342-390` forbids inventing implicitly; `--border` is deliberately not a legal pair, so a generated value would have nothing to verify against; `--input` is D3; `--secondary` is unchanged by anything in the parity target. `--muted`/`--muted-foreground` is named as the *only* candidate a future request could carry cheaply — the `muted-surface` role and its two 7:1 pairs already exist and the emitter derives its lists from `TOKEN_POLICY` so `light_only` stays correct — but it is not taken here. Recorded as a standing precondition: because `getOrgBrandForLayout()` regenerates unconditionally and ignores the stored `brand_token_version` (`read-org-brand.ts:137`, `contract.ts:33-49`, `docs/TODO.md:145`), any future widening changes every branded org's live colours on the next page load with no migration and no audit row — the read/write asymmetry must be resolved before or with any partition change, not after. The platform-wide `--radius` bump to 0.625rem is separately ruled out of this track: it repaints every route group and would swamp the `maxDiffPixels: 0` visual harness that is Track B's only regression signal; if wanted, a separate Polish-class work-log after Track B ships.

---

**DECISION-141: A second-round external post-merge review is closed by generalizing F44/F49's own rule — a grant edit never binds `getPlatformDb()`, only a trigger does — from the lifecycle/affiliation/returns pipeline's *mutation* surfaces to its remaining *creation* surfaces. `organization_lifecycle_events` and `organization_successions` become one GUC-gated aggregate (`presby.lifecycle_write_active`); the return→publication→projection chain (`statistical_returns`/`publications`/`congregation_statistics`'s `published_by_congregation` branch) shares one GUC (`presby.publication_write_active`) armed inside `presby_publish_sasr_snapshot()`; the withdrawal pair is gated behind a third, distinct GUC (`presby.withdrawal_write_active`) that nothing sets yet; `organization_identifiers` INSERT closes under its existing GUC for consistency.** (2026-09-24, tech-lead Phase 3, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`, sixth loop-back, ruling on a second external review's five findings, F54–F58.)

**The standing rule, stated once so it isn't re-derived per table:** if an immutable record represents an authorized act, its `INSERT` needs the same GUC-gated-trigger treatment its `UPDATE`/`DELETE` already gets — a revoked grant or a shape `CHECK` proves the row is *well-formed*, never that it arrived through the *sanctioned path*, and only a trigger reading a transaction-local marker distinguishes the two on the one connection (`neondb_owner`, via `getPlatformDb()`/`PLATFORM_DATABASE_URL`) that no grant can bind (F44).

**Three GUC decisions, made explicitly so the reuse-vs-new choice isn't re-litigated per table.** (1) `presby.lifecycle_write_active` is shared across `organization_lifecycle_events` and `organization_successions`, both gated by one new guard function reusing the **existing** `presby_deny_lifecycle_change()` literal — one future writer (`presby_record_lifecycle_event()`), one transaction, one claim, so this is the affiliation-table reuse case (DECISION-139/140), not the identifier-table no-reuse case. (2) `presby.publication_write_active` is likewise **one** GUC across three tables and, deliberately, across both `statistical_returns` provenances (`submitted` today, `imported` once D13's import function ships) rather than a second `presby.import_write_active` — the marker records "this transaction is part of one sanctioned publish operation," a claim that does not vary by provenance [corrected 2026-09-25, twelfth loop-back, from "the guard's job is proving 'a sanctioned function wrote this'" — GUC = workflow marker; privilege = authority boundary, per F63; the marker alone proves nothing about who wrote the row, only what the transaction claims to be doing], and a two-GUC OR would be complexity without additional protection. On `congregation_statistics` the new guard is `WHEN (NEW.provenance = 'published_by_congregation')`-scoped specifically so the live `presbytery_entered`/`imported` tenant-DML path (`setCongregationStatisticsAction`) never reaches it — the mechanism that lets this decision close a real hole without touching a shipped, member-facing write path. (3) `presby.withdrawal_write_active` is a **new**, unshared GUC — the future `presby_withdraw_publication()` runs in a different transaction, at a different point in time, than the publish path, so there is no transaction-local claim to piggyback on; this is the identifier-table's no-reuse case.

**Zero-regression verified, not assumed, before ruling.** No application code writes `organization_lifecycle_events`, `organization_successions`, `statistical_returns`, or `publications` today outside `presby_publish_sasr_snapshot()` and `drizzle/0047`'s own backfill — confirmed by search, including `createOrganization()`, which writes only the initial affiliation, never a lifecycle event, and `scripts/seed-dev.sql`, which touches neither table. Every guard added by this decision is real risk closed with no live caller to break; the entire implementation cost lands on updating direct-`INSERT` test fixtures (`test-rls.sql`, `lifecycle.test.ts`) to arm the new GUCs themselves, not on any production write path.

**Two items ruled informational, not mechanism changes.** Attestation identity (`statistical_returns.attested_by_name`/`attested_role`, `organization_lifecycle_events.recorded_by`) is a documentation correction to D21/F50's text — the database proves a submission was attested *at a time*, not *by whom*, until an `app.current_user_id` GUC exists — folded into the existing TODO line, not a new mechanism. `organization_identifiers` INSERT (F47's accepted residual) is closed anyway, at near-zero cost, because the sole sanctioned writer already arms the relevant GUC unconditionally at entry.

**Correction (2026-09-25, tech-lead Phase 3, seventh loop-back — ratification after hardening round two, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`):** the zero-regression paragraph above is wrong about the seed. `scripts/seed-dev.sql` does **not** touch `organization_lifecycle_events`/`organization_successions`, but it does write the publication chain — `statistical_returns` (`:1374`), `publications` (`:1386`), and a `published_by_congregation` `congregation_statistics` row (`:1399`), all inside the file's one transaction — so "scripts/seed-dev.sql, which touches neither table" overclaimed across all four tables when it was only ever true of the two lifecycle ones. Caught by the implementer running the seed, not by re-reasoning; the seed now arms `presby.publication_write_active` once, immediately before those three inserts. Built as specified otherwise: `presby_publish_sasr_snapshot()` and `drizzle/0047`'s backfill are the other two arming sites, so `presby.publication_write_active` is armed at three sites total, not two — the canonical list is recorded in `docs/schema-design-2.md` §2g. Two further items ratified as built rather than as specified: (1) `presby_freeze_publication()`'s new sanctioned-writer rejection is a fifth **in-function** literal (not a shared helper) — F40's uniform-literal rule is inapplicable here (it guards against an existence oracle for a caller who does *not* already hold the row; every branch of this function fires on an `UPDATE ... WHERE id = old.id`, so the caller already named the row), and `publications` now carries five distinct per-reason literals through this one function, which is the correct discipline for a mutation surface reached only by an id the caller already possesses. (2) The orchestrator's mid-task acceptance of revoking `presby_app`'s unused `INSERT` on `organization_lifecycle_events` (Finding 1) is ratified as Ruling-1-consistent — the marker-is-not-a-privilege gap it closes was real and the revoke costs no live caller — and the coupling it creates (`presby_deny_lifecycle_change()` and `presby_lifecycle_event_cardinality_check()`'s `presby_app` EXECUTE grants must return together with any future re-grant of that INSERT) is confirmed recorded at both durable locations a future implementer will read: inline at both revoke sites in `drizzle/0044`, and in the work-log's Handoff section. See `docs/schema-design-2.md` §2g for the full ratification, including the residual left open on `congregation_statistics` (the `WHEN`-scope boundary, accepted) and the disposition of Finding 4a (orphan-fixture guard predicate left unchanged, deferred to its own scoped pass).

**Correction (2026-09-25, tech-lead Phase 3, eighth loop-back — ruling on QA's re-verification FAIL, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`):** QA's re-verification of the round-two build found every suite green but two of this decision's own claims false as stated, both because a transaction-local GUC was treated as sufficient on its own where it needed a companion mechanism. **`presby.lifecycle_write_active`** does not, as F54's text and `drizzle/0044:1338` claimed, stop "a later, separate INSERT... into an already-valid, already-committed merged event" — a later transaction can re-arm the same boolean sentinel and append to a settled event; no live exposure (`presby_app` holds `SELECT`-only on both tables), but false for the owner connection the guard exists to bind. Ruled: the GUC now carries the specific event id, not a boolean, and the guard compares the row being written against it — closing the case where the *marker is armed but names no act*, but **not** closing the case where a later transaction re-arms it to a pre-existing event's own id, which is declined as requiring machinery (a `SECURITY DEFINER` same-transaction/`xmin` check, unprecedented in this schema) disproportionate to a residual bounded to owner-level connections that can already bypass any trigger here by disabling it (F44's own standing fact, extended explicitly to name this consequence). **`presby.withdrawal_write_active`** does not make the withdrawal transition "unreachable on every connection until the writer ships" (`drizzle/0047:323`) — `presby_app` holds whole-table `UPDATE` on `congregation_statistics` from `drizzle/0038:280`, never narrowed by F55/F56, and a GUC is `set_config`-able by any role, so `presby_app` can arm it and write `withdrawn_at` itself. Ruled: `presby_app`'s `INSERT`/`UPDATE` on `congregation_statistics` are narrowed by **column-level grant** to every column except `withdrawn_at` and `publication_id` — the correct instrument for "this role must never touch these two columns," which no trigger can provide on its own — with `presby_platform` untouched (F47 precedent). This also closes, in full rather than merely narrows, the residual the seventh loop-back's ratification accepted (Finding 1's `congregation_statistics` `INSERT` gap): `setCongregationStatisticsAction` never sets `publication_id`, so revoking `INSERT` on that column costs the live write path nothing. Full ruling, the coverage disposition, and the punch list for database-admin's ninth Phase 4 pass are in the work-log's "Ruling on QA-1/QA-2 after hardening round two" and `docs/schema-design-2.md` §2g's matching correction.

**Build note (2026-09-25, database-admin, ninth Phase 4 pass):** the `UPDATE` half shipped as ruled; the `INSERT` half did not, and the reason is measured rather than argued (F59, `docs/schema-design-2.md`; `drizzle/0047` section 10). Postgres requires column-level `INSERT` privilege on every column in the INSERT *target list* even when its value is the `DEFAULT` keyword, and drizzle-orm 0.45's insert builder emits **every** column of the table with `default` for the unspecified ones — so a column-level `INSERT` revoke on any column of `congregation_statistics` breaks `setCongregationStatistics()`, the shipped write path this ruling required to keep working (built first, then measured: `src/lib/presbytery.test.ts` 5 red, two `permission denied` on that upsert; removed, then 34/34). The seventh loop-back's item-7 `INSERT` residual therefore **stands** rather than closing, bounded as before by `congregation_statistics_publication_shape` and the composite FK to `publications`. Closing it is an api-developer change (raw parameterised SQL for that one insert), not a migration.

**Correction (2026-09-25, tech-lead Phase 3, tenth and final loop-back — ruling on F59 and QA's Phase 5 re-verification Finding 1, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`):** F59 is **accepted as built** — the `UPDATE` half stands as the fix for the measured QA-2 defect, and the `INSERT` half is not buildable without breaking `setCongregationStatistics()`, per the ninth pass's build note above. Two corrections to how the accepted `INSERT` residual is described, both caught by QA rather than reasoned from docs: (1) the "composite FK to `publications`" credited above (and in `drizzle/0047` §10 and `docs/schema-design-2.md` F59 before this correction) is the **wrong** one of two — `congregation_statistics_publication_fk (publication_id, about_org_id)` pins the row's *source* congregation, not which council may insert; the FK that actually produces "already addressed to the inserting council" is `congregation_statistics_publication_recipient_fk (publication_id, organization_id) → publications (id, recipient_org_id)`. (2) The bound constrains *which publication* is named, not the row's content — QA inserted a fabricated `published_by_congregation` row for report year 2024 against a real 2025 publication, with an arbitrary `ending_active` and `minute_reference`, and it inserted cleanly (no collision, since `congregation_statistics_entered_unique_idx` is partial) and was then frozen permanently. Stated honestly, the residual is: a recipient council on the tenant connection, self-arming `presby.publication_write_active`, can manufacture a permanent projection row for a year the congregation never published, bounded only to publications actually addressed to it. Both corrected in place in `drizzle/0047` §10 and `docs/schema-design-2.md` F59. **The closing instrument, if the residual is ever judged worth closing, is named definitively:** move `setCongregationStatistics()`'s write off Drizzle's insert builder onto an explicit column-list raw SQL insert (naming only the columns it sets), an api-developer change in `src/lib/presbytery.ts` — chosen over a `SECURITY DEFINER` function because it is the only tenant-connection Drizzle `insert()` target on this table today (`presby_publish_sasr_snapshot()` already runs `DEFINER` and is grant-exempt per F44; `scripts/seed-dev.sql` writes raw SQL), so this one call site alone would fully satisfy F59's general rule with no new privilege-elevation surface to review. Tracked in `docs/TODO.md`, not built here. This is the last correction on `drizzle/0043`–`0047` — no further design defect is expected on these migrations.

**Correction (2026-09-25, tech-lead Phase 3, eleventh loop-back — ruling on a third external post-merge review, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`):** the tenth loop-back's "last correction on `drizzle/0043`–`0047`" sentence is superseded by the operator's decision to run one further external review; nothing else about the tenth ruling is disturbed. **F59 is reopened and closed, not merely reaccepted.** A third-round reviewer judged the accepted `INSERT` residual worth closing rather than leaving open — `congregation_statistics` feeds operational calculations, and an armed marker plus a real-publication FK is not proof the projection's *content* matches what was published. Ruled: `setCongregationStatistics()` (`src/lib/presbytery.ts`) moves off Drizzle's insert builder onto an explicit-column raw SQL upsert, and the `INSERT` grant becomes column-level — catalog-generated like the `UPDATE` grant — excluding `publication_id`, `withdrawn_at`, and (newly, beyond the ninth pass's abandoned attempt) `published_at`, since the live `presbytery_entered` path never names it and the shape `CHECK` does not reach it. This is buildable where the ninth pass's identical-looking attempt was not, because the fix now also changes the *call site* off Drizzle's all-column insert builder — F59's actual blocker (Postgres requiring privilege on every `DEFAULT`-valued column in an all-column target list) never applied to a target list that only names the columns actually set. The `docs/TODO.md` line tracking this as a deferred "if ever judged worth closing" item is retired; the fix ships in this pass. Two further items from the same review: (F60) all 21 `SECURITY DEFINER` functions in `drizzle/0043`–`0047` gain `pg_temp` appended to their existing `search_path = public` clause — demonstrated, not theoretical, per a concurrent security-pipeline probe that forged a `SECURITY DEFINER` function's answer via an attacker-created temp table shadowing an unqualified relation; (F62) the two deferred lifecycle-cardinality wrapper trigger functions become `SECURITY DEFINER`, closing a `permission denied for function` failure mode this file's own section 13b comment already predicted would recur once `presby_record_lifecycle_event()` ships. Full ruling and the implementer brief: `docs/schema-design-2.md` §2h and the work-log's "Ruling on the third external review (F60–F64)".

**Status:** Resolved · **Date:** 2026-09-24 · **Feature:** `2026-09-24-lifecycle-affiliation-returns` (Phase 3, sixth loop-back; build ratified with one factual correction at the seventh loop-back, 2026-09-25; two further claims corrected and routed to database-admin's ninth Phase 4 pass at the eighth loop-back, 2026-09-25; F59 ruled accepted-as-built and its residual's description corrected at the tenth loop-back, 2026-09-25; F59 reopened and closed, plus F60/F62, at the eleventh loop-back, 2026-09-25 — tenth Phase 4 pass pending)

---

**DECISION-140: The fourth Phase 3 loop-back on the lifecycle/affiliation/returns pipeline closes five external-review findings (F47–F52) as a migration-correction-in-place, with one standing rule applied uniformly: a grant edit is never sufficient against `getPlatformDb()` (F44) — every real fix here is a trigger, keyed off either a reused transaction-local GUC or a new one, never a `revoke` alone.** (2026-09-24, tech-lead Phase 3, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`, ruling on an external reviewer's read of the exported table shapes.)

Five mechanism choices, named so they aren't re-litigated: (1) `organization_identifiers` gets its own new GUC (`presby.identifier_trigger_active`), not a reuse of the affiliation GUC — different table, different transaction, no shared claim to piggyback on. (2) `organization_affiliations`, by contrast, **reuses** `presby.affiliation_trigger_active` (moved earlier in `presby_transfer_affiliation()`) for its own new UPDATE/DELETE guard, because it is the same function, same transaction, same "sanctioned path" claim the `organizations` reparent guard already keys off — reuse there is the minimum-complexity answer, not a false economy. (3) The DELETE arm of that same guard cannot unconditionally freeze like `organization_successions` does, because `subject_org_id`'s `ON DELETE CASCADE` exists specifically to let a `deletable_until`-stamped fixture teardown clean up — so `presby_guard_organizations_delete()` now sets the GUC itself once it has validated the fixture window, pre-authorizing the cascade without touching any of the 15+ existing test-teardown call sites. (4) The projection's recipient end (`congregation_statistics.organization_id` vs. `publications.recipient_org_id`) is proven by a **second composite FK** anchored on a new `UNIQUE (id, recipient_org_id)` on `publications`, rather than a trigger — both FKs pin to the same `publications.id`, so no additional runtime check is needed once the two FKs exist. (5) Withdrawal semantics are ruled **Option A** (retain as historical record, exclude from current calculations by column) throughout, reversing the read-function's just-shipped Option-B-shaped filter rather than leaving the inconsistency the review found between the read path and the projection.

One correction to the review's own diagnosis, load-bearing across all five: the review treats grant-narrowing as sufficient for `organization_identifiers` and would, by omission, have accepted it as sufficient anywhere else in this batch — but this pipeline's own F44 already established that `PLATFORM_DATABASE_URL` authenticates as `neondb_owner`, which bypasses every grant by table ownership. Every ruling that closes a real hole in this decision does so with a trigger; grants are documentation of eventual intent for a real non-owner `presby_platform` login, never the enforcement mechanism.

**Correction (2026-09-24, tech-lead Phase 3, fifth loop-back — ratification after the hardening pass, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`):** two of this decision's specified mechanisms did not survive contact with the existing tree unchanged, and both are ratified as built rather than reworked. First, the fourth `organization_affiliations` CHECK this decision implied (`effective_from` required on every `'recorded'` row) is refused, not added: `scripts/seed-dev.sql`'s Quillhaven row and `scripts/test-rls.sql`'s F41 assertion both depend on a minuted `'recorded'` row whose beginning predates the records, and F41's null-means-unbounded was never conditioned on `authority`. The absence is pinned by a `test-rls.sql` assertion rather than silently missing. Second, `statistical_returns_provenance_shape`'s `submitted` branch does not require `attested_by_name`/`attested_role` non-null, only `attested_at` — the literal predicate would have made every live `presby_publish_sasr_snapshot()` call fail, since that function writes both text columns NULL on purpose (Ruling A4, no `app.current_user_id` GUC). Ratified as the Ruling-A4-consistent shape; tightening those two columns is folded into the existing GUC TODO line, not a separate one. See `docs/schema-design-2.md` §2f F49/F50 for the corrected text.

**Status:** Resolved · **Date:** 2026-09-24 · **Feature:** `2026-09-24-lifecycle-affiliation-returns` (Phase 3, fourth loop-back; ratified with two corrections at the fifth loop-back)

---

**DECISION-139: `presby_transfer_affiliation()`'s standing check is two cases, not a flat three-way OR — same parent = actor must be that parent; different parents (a true transfer) = actor must be the recorded common superior of both, checked via `presby_affiliation_parent_as_of()`, never `organizations.parent_id`. Every rejection anywhere in the affiliation/lifecycle write path raises one literal string per table via a shared `presby_deny_affiliation_change()` helper. `presby_org_affiliated()` and `presby_affiliation_parent_as_of()` both ship in increment 2 (0044) rather than increment 3 (0045), and increment 3 is narrowed to the enforcing trigger alone. `createOrganization()`'s affiliation insert derives `authority` from whether a minute reference was supplied, never taking it as a separate caller-supplied enum.** (2026-09-24, tech-lead Phase 3, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`, turning Phase 2 Rulings 1/2/4 into buildable algorithms.)

**First, the standing algorithm.** Ruling 1's prose ("the actor must be the current parent, the incoming parent, or a council above both") reads as a flat three-way OR, but taken literally it would let either presbytery in a redistricting unilaterally move a congregation to the other — exactly the failure Flow 2 exists to prevent. Read together with Ruling 1's own next sentence ("for a true redistricting... the actor must be the common superior"), the coherent rule is two cases: when `p_new_parent_org_id` equals the subject's current parent (a correction, not a jurisdictional change — fixing a `relationship_type` typo, or promoting a backfilled row to a minuted one), the acting council must BE that parent; when the two parents differ, the actor must be the recorded common superior of both as of the effective date — never "either party alone." This is what makes a synod, not either presbytery, the only actor who can execute D19's headline scenario, and it is checked against `presby_affiliation_parent_as_of()` (a real read of the affiliation history) rather than `organizations.parent_id` (the cache the same transaction is about to rewrite — circular if used as authority).

**Second, the uniform-message discipline (F40) needed a concrete shape to be enforceable rather than aspirational.** A single helper function, `presby_deny_affiliation_change()`, raising one literal string and `errcode = 'insufficient_privilege'`, called from every rejection branch in `presby_transfer_affiliation()`, means the string exists in exactly one place and cannot drift as the function grows failure branches over time — the alternative (each `raise exception` call typing the string inline) is exactly how DECISION-040's byte-identical discipline erodes silently across future edits. `organization_lifecycle_events` gets its own literal (a different table, no EXCLUDE constraint, so F40's specific oracle risk doesn't apply there) rather than sharing the affiliation table's string, so a future reader can't assume the two tables' rejection surfaces are interchangeable.

**Third, sequencing `presby_org_affiliated()`/`presby_affiliation_parent_as_of()` into increment 2.** `docs/schema-design-2.md`'s literal migration order puts `presby_org_affiliated()` in increment 3 ("(3) `presby_org_affiliated()` and the about-org trigger"). But `presby_transfer_affiliation()` — increment 2's own function — needs `presby_affiliation_parent_as_of()` for its standing check, and the two functions are near-identical reads over the same table created in increment 2. Shipping both in 0044 and narrowing 0045 to just the enforcing trigger avoids a function ordering dependency across migration files for no isolation benefit — the design doc is corrected accordingly (see the §2b/§3 edits in this same commit).

**Fourth, `authority` is derived, not asked for.** `createOrganization()`'s extended input takes an optional `minuteReference`; the affiliation insert sets `authority = 'recorded'` when one is supplied and `'backfill'` when it is not, rather than exposing `authority` as its own caller-settable field. This makes the CHECK constraint's own invariant (`authority = 'backfill' OR minute_reference IS NOT NULL`) impossible for a caller to violate by construction, rather than merely validated after the fact.

**Correction (2026-09-24, tech-lead Phase 3 amendment after batch A):** the standing algorithm above is right, but this decision's literal function signature was not buildable and its literal authority-check wiring would have defeated its own uniform-message rule. `presby_transfer_affiliation(p_subject_org_id, p_new_parent_org_id, p_relationship_type, p_effective_on, p_reason default null, p_minute_reference, p_concurrence_reference default null)` places a defaulted parameter (`p_reason`) before a non-defaulted one (`p_minute_reference`) — Postgres rejects that outright. `p_minute_reference` moves ahead of `p_reason` in the built signature (it is required in practice anyway, since the row it writes carries `authority = 'recorded'`). Separately, this decision has `presby_assert_council_authority()` called as `(actor, subject)` with a flat one-level-above rule and always raising the lifecycle literal — both wrong, and the second is the more consequential defect: on an affiliation row the one-level rule is between `parent_org_id` and `subject_org_id` (parent-fits-child), not between the *actor* and the subject, because a redistricting's actor (a synod) and subject (a congregation) are two levels apart by design — the actor's standing is D19's own common-superior check above, a different rule entirely. Conflating the two would have made D19's headline scenario (the synod-on-presbytery/congregation redistricting this decision exists to enable) rejected by the very function meant to enable it. The built algorithm is the correct one and is adopted as this decision's text going forward: `presby_assert_council_authority(p_actor_org_id, p_subject_org_id, p_context)` checks parent-fits-child when called from the affiliation path (actor argument = the new/current *parent*, not the transfer's session actor) and actor-fits-subject when called from the lifecycle path (`p_context = 'organization_lifecycle_events'`), and `p_context` selects which table's uniform-rejection literal it raises — without it, a rejection from inside the affiliation path would have raised the lifecycle table's text, making one cause distinguishable from another and defeating F40's whole point. See `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md` Phase 3's amendment section for the full ruling.

**Status:** Resolved · **Date:** 2026-09-24 · **Feature:** `2026-09-24-lifecycle-affiliation-returns` (Phase 3)

---

**DECISION-138: The lifecycle/returns tables land as three new domain modules (`lifecycle.ts`, `publication.ts`, `returns.ts`) with `reporting.ts` deleted, one migration file per §2b increment (`drizzle/0043`–`0049`), and the polity vocabulary corrected — the lifecycle enum is G-3.0301**(a)**, not (c); the reviewing-council rule is G-3.0108(a), not G-3.0107; `divided` is kept with a stated scope note (our addition for congregations, verbatim polity for presbyteries and synods); and `relationship_type` gains `member_nwc`.** (2026-09-24, architect Phase 2, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`.)

`publications` gets its own module rather than living in `returns.ts` because D20 defines it as generic over `record_class`: parking the schema expression of the "access flows up by publication" invariant inside a statistics module guarantees that the second record class either moves the table between files or leaves the module name lying. `organization_identifiers` and `organization_name_history` extend `org.ts` instead — both are organization identity (public, not council-owned), sitting beside `organizations` the way `person_identifiers` sits in `people.ts`, and `org.ts` stays under 350 lines. `reporting.ts` is deleted with `sasr_reports` rather than left as a one-table husk. On the polity: verification against the full 2023–2025 Book of Order text (not a search paraphrase) found that *"organizing, receiving, merging, dismissing, and dissolving congregations in consultation with their members"* is **G-3.0301(a)**; G-3.0301(c) is the ministers-of-Word-and-Sacrament list, which separately supplies *"taking jurisdiction over the members of dissolved congregations"* — the polity basis for what happens to a dissolved congregation's roll, and worth citing where the dissolution trigger is designed. G-3.0107 is *Records*, whose real contribution here is *"when a council ceases to exist, its records shall become the property of the next higher council"* (the argument for filtering the recipient read on the publication rather than on a live affiliation); the annual-review rule is G-3.0108(a). The wrong subsection had been cited three times in `docs/schema-design-2.md` and cross-checked as "correct" by round 3, which is the argument for verifying a citation against the constitution's text rather than a second agent's summary. `divided` survives the check that it is absent from G-3.0301(a) because congregations do divide in fact and 1→N is why D10 has a topology table at all — and because for presbyteries (G-3.0403(c)) and synods (G-3.0502(d)) "dividing" is verbatim polity, so the value is scoped, not invented. `member_nwc` is added rather than overloading `member_congregation`: G-3.0301(b) enumerates "new church developments … and other non-congregational entities" separately from congregations, and conflating them would make the presbytery's congregation count — a live per-capita and commissioner-parity number — wrong.

---

**DECISION-137: The `congregation_statistics` retrofit is narrower than round 3's R3.4(a) specified — `supersedes_publication_id` moves to `publications`, but `minute_reference` and `published_at` stay on the projection; `sasr_reports` is dropped outright, and the working-draft need it nominally served becomes a tracked question for the publish-UI pipeline.** (2026-09-24, architect Phase 2, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`.)

Read against the code rather than the design doc, two of R3.4(a)'s three column moves break shipped behavior (F39). `minute_reference` is written on `provenance = 'presbytery_entered'` rows by `setCongregationStatistics()` (`src/lib/presbytery.ts:637,664`) and is a **required** field on the live presbytery-entry form (`src/app/(org)/o/[slug]/admin/reports/statistics-schema.ts:22`) — a row with that provenance has no publication to hold it. They are also two different facts: the presbytery's minute for its own data entry, and the congregation's session minute authorizing publication. Each table keeps its own. `published_at` is ordered on at `src/lib/presbytery.ts:522` and drives the provenance coalesce at `:533-539`; moving it to `publications` puts it in a table the presbytery cannot read (`publications.organization_id` is the source congregation), turning the shipped rollup and the per-capita basis-year lookup into SECURITY DEFINER joins for no gain. A projection is allowed to carry the event's facts — that is what a projection is — and unlike F29's `current_roll` this copy cannot drift: both rows are written by one DEFINER function in one transaction and neither is ever updated, the `congregation_statistics_freeze` trigger (`drizzle/0038:286-296`) already guaranteeing the second half. A `test-rls.sql` assertion that every published row's `published_at` equals its publication's makes the non-drift checkable rather than argued. `supersedes_publication_id` does move — no reader exists outside a comment. `sasr_reports` is dropped: zero rows on both the production and development branches, zero application consumers, and a shape (`status: draft | session_approved | submitted`, mutable `payload` with `$onUpdate`, `src/lib/db/domain/reporting.ts:47,54,60`) already stale under D25 — keeping a real table with no consumer and a known-wrong shape is the second-source-of-truth pattern this design refuses everywhere else. What drops with it is real and is recorded rather than elided: the congregation loses any staging surface between computing the projection and attesting it, and designing one against D25's rules is the publish-UI pipeline's job, tracked in `docs/TODO.md` in the same commit.

**Correction (2026-09-24, tech-lead Phase 3 amendment after batches B and C, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`):** this decision's own retrofit FK, as first specified, named the wrong column and would have rejected every row it exists to protect. `foreign key (publication_id, organization_id) references publications (id, organization_id)` compares `congregation_statistics.organization_id` (the **recipient presbytery**) against `publications.organization_id` (the **source congregation**) — different organizations by construction (DECISION-120 meets D20), so the constraint would fail on the first published row. The column that actually equals `publications.organization_id` is `congregation_statistics.about_org_id`: the built and adopted shape is `foreign key (publication_id, about_org_id) references publications (id, organization_id)`, which preserves F2's guarantee rather than weakening it — a projection still cannot claim a publication recorded by a different congregation. Only the column name in the original spec was wrong; the retrofit's narrowing (this decision's headline) is otherwise unchanged.

**Second correction (2026-09-24, tech-lead Phase 3 fourth amendment, external post-merge review, F51/DECISION-140):** the batches-B/C correction above proves the *source* end of the projection's relationship (`about_org_id` = `publications.organization_id`) but nothing proved the **recipient** end — `congregation_statistics.organization_id` (the recipient presbytery) could still point, via `publication_id`, at a publication whose actual `recipient_org_id` was a *different* presbytery, provided the source congregation matched. Closed with a second composite FK: `publications` gains `UNIQUE (id, recipient_org_id)`, and `congregation_statistics` gains `foreign key (publication_id, organization_id) references publications (id, recipient_org_id)`, alongside the existing source-end FK. Both FKs share `publication_id` and pin to the same `publications.id`, so together they now prove both ends of the same row. See `docs/schema-design-2.md` §2f F51.

---

**DECISION-136: `organizations` becomes write-closed to `presby_app` by an explicit revoke, because the live database contradicts the documented grant model — and hierarchical provisioning moves into `createOrganization()` rather than being deferred, with direct `INSERT ... parent_id` rejected (never auto-creating an affiliation) and the affiliation backfill given an unbounded-below lower bound.** (2026-09-24, architect Phase 2, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`.)

`drizzle/0009_presby_rls.sql:93` grants `select` on `organizations` to `presby_app` and the design's entire "nothing writes `parent_id`, `path` or `lifecycle_status` directly" story rests on that. The database says otherwise: querying the development branch, `presby_app` holds SELECT, **INSERT, UPDATE and DELETE** on `organizations`, a table with `relrowsecurity = f` and no triggers at all (F38). The mechanism is that `0009:93` is additive and never revoked anything, while a blanket platform-table grant — genuinely needed, since `src/auth.ts:79` runs the NextAuth Drizzle adapter on the `presby_app` connection — swept `organizations` in alongside `users` and `feature_flags`; `people` (`0009:376`) is the only table in the whole `drizzle/` tree ever explicitly clawed back. No application path exploits this, but "the trigger must be SECURITY DEFINER for grant reasons" is currently a statement about a migration file rather than about the database, and `revoke delete` for the D10 guard is therefore not the no-op it looks like. Increment 2 issues `revoke insert, update, delete on organizations from presby_app` and `scripts/test-rls.sql` asserts the resulting grant shape directly, in the style section 29 already uses (`scripts/test-rls.sql:2118-2126`). The derived-column write is **one** SECURITY DEFINER function, `presby_apply_affiliation_to_org_tree()`, called by both the affiliation and lifecycle triggers — the recursive subtree path rebuild is the most error-prone part of D19 (`deriveOrgPath` handles roots only today) and must not exist in two copies; the `BEFORE UPDATE OF parent_id, path` guard checks a GUC that function sets transaction-locally. Provisioning extends `createOrganization()` (`src/lib/org-provisioning.ts:159-288`), already the single write path for `organizations` and already running on `getPlatformDb()`, which the revoke does not touch: an optional parent inserts the org row and its initial affiliation in one transaction, owned by the parent council with `recorded_by` carrying the operator. A presbytery-callable `presby_organize_congregation()` is deferred to the lifecycle-UI pipeline, which is also where D13's import resolution needs it — one function, built once. Direct `INSERT ... parent_id` is **rejected**, not auto-resolved into an affiliation row: auto-creation would have to invent the owning council, the effective date and the authority, minting minute-less rows and making `parent_id` a write target again. The regression is bounded because the rejection fires only when `parent_id is not null` and the trigger GUC is absent, leaving the ~20 root-org test fixtures untouched; the actual sweep is five files (`scripts/seed-dev.sql:30`, `scripts/test-rls.sql:1997/2155/2322`, `src/lib/credentials.test.ts:128`, `src/lib/presbytery.test.ts:122`), and the three `test-rls.sql` inserts break on the revoke regardless. Finally, the backfill's `effective_from` must be **null (unbounded below) with an explicit backfill marker**, not `now()` or `created_at` (F41): a bounded lower bound makes `presby_org_affiliated(as_of => <report year>)` false for all 41 years of PSV history, so the about-org trigger from increment 3 would block the very import increment 7 exists to enable. A CHECK permits a null `minute_reference` only on a backfilled row, so an inferred relationship can never pass for a minuted act.

**Correction (2026-09-24, tech-lead Phase 3 amendment after batches B and C, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`):** `PLATFORM_DATABASE_URL` — the `getPlatformDb()` connection this decision's revoke was written around — authenticates as `neondb_owner`, a member of both `presby_platform` and `presby_app` who holds every privilege on every table by **ownership**, independent of any `grant`/`revoke` this decision or DECISION-135/Ruling A5 makes. The revoke and the grant-narrowing are still correct and still worth doing — as the statement that binds the day a real, non-owner `presby_platform` login role exists, and as documentation of intent a future reader should not undo — but on the connection that matters *today*, the only real backstop is a trigger (`presby_guard_organizations_delete()`, `presby_freeze_lifecycle_event()`, and their siblings), never a grant. Standing fact for every future design reasoning about `getPlatformDb()`'s reach (`docs/schema-design-2.md` §2d F44). Separately: `drizzle/0009:44`'s blanket `grant ... on all tables ... to presby_platform` remains this decision's and F38's unremoved root cause, and by the end of this pipeline it touches five narrowed tables (`organizations`, `organization_affiliations`, `organization_lifecycle_events`, `statistical_returns`, `publications`) rather than one — a re-application of `0009` out of migration order would re-widen all five. Flagged for the next security review, not fixed here.

---

**DECISION-135: Cross-council writes and reads on the three-axis organization tables are function-mediated, never policy-mediated — `presby_transfer_affiliation()` in the `presby_publish_sasr_snapshot()` shape, with `organization_affiliations` DML revoked from `presby_app`, affiliation-row ownership treated as immutable provenance, and a recipient-side DEFINER read shipped in the same pipeline as `publications`.** (2026-09-24, architect Phase 2, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`, resolving Phase 1's Flow 1 and Flow 2.)

Phase 1 found that D26's council-owned affiliation rows make D19's own headline scenario unexecutable: under ordinary `tenant_isolation`, a synod can neither close Presbytery A's open row nor open Presbytery B's, because its context is neither. Council ownership is nonetheless kept — `reason`, `minute_reference` and `notes` are genuinely the acting council's records, and a neutral "registrar" owner would need either a synthetic tenant (a fourth axis) or a world-readable table carrying the text explaining why a church was dissolved, the hazard already named for putting tenant data on `organizations`. What changes is that the consequence is stated as a rule instead of being worked around case by case: cross-council access to these tables goes through SECURITY DEFINER functions, never through a policy. `presby_transfer_affiliation()` accepts **no acting-council id** — the actor is `presby_current_org()`, already membership-verified by `withOrgContext()`, the strengthened confused-deputy form `drizzle/0038:376-380` established — verifies standing from the affiliation history itself rather than from `organizations.path`/`parent_id` (the derived cache the same transaction is about to rewrite, so reading it as the authority is circular), requires the actor to be the common superior council for a two-parent transfer (G-3.0403(c) for presbyteries, G-3.0502(d) for synods, with GA concurrence per G-3.0502(e) modeled as a nullable reference rather than a second actor), and closes plus opens atomically before calling the single tree-derivation function. `insert/update/delete` on `organization_affiliations` is revoked from `presby_app` so that function is the only writer — which is necessary, not tidy: unique and EXCLUDE constraints are enforced against all rows regardless of RLS, so with ordinary DML a presbytery could probe `insert (subject, parent => itself, …)` and learn from the constraint error whether an organization it cannot see currently has an open affiliation, DECISION-047's enumeration oracle arriving through a constraint instead of a page (F40). With the function as sole writer, every rejection returns one uniform message. A closed row keeps the `organization_id` of the council that recorded it — ownership is provenance and never changes; re-owning it to the closing synod would rewrite who acted — and gains `closed_by_org_id`/`closed_by`/`closed_on`/`closed_minute_reference` so the closing act stays attributable, the close itself being a DEFINER write into another council's space exactly as `presby_publish_sasr_snapshot()` already writes a presbytery-owned row from a congregation's context. Reads follow the same idiom: `presby_org_affiliated(subject, council, as_of)` and `presby_affiliation_parent_as_of(subject, as_of)` cross owners for the archive, and `presby_list_published_returns_to_me()` — mirroring `presby_list_own_congregation_publications()` (`drizzle/0038:665-690`) to the recipient side — ships in the same increment as `publications`, because without it D20's "a recipient council reads the artifact published to it" is unenforceable in the permissive direction: the presbytery simply cannot read a `statistical_returns` row the congregation owns. That function filters on `publications.recipient_org_id` rather than on a live affiliation, on polity grounds: G-3.0107 makes a ceased council's records the property of the next higher council, so a presbytery must still read a dissolved congregation's returns after dissolution. None of this is a bespoke RLS policy, so `docs/schema-design.md` §17's rule that exactly two named cross-org policies exist (`person_links`, `transfer_certificates`) stands, and DECISION-112's refusal of a third is not reopened.

**Third correction (2026-09-24, tech-lead Phase 3 fourth amendment, external post-merge review, DECISION-140):** function-mediation had not yet reached every table this decision's own principle covers. `organization_identifiers` (added in the same pipeline's increment 1, `drizzle/0043`) shipped with unrestricted `presby_app` DML and no DEFINER writer at all — now closed the same way as `organization_affiliations`: `presby_app` to `SELECT`, a new `presby_set_organization_identifier()` DEFINER function as sole tenant-side writer, standing checked via `presby_org_affiliated()`. `statistical_returns` and `publications` still granted `presby_app`/`presby_platform` direct `INSERT` alongside their DEFINER writer (`presby_publish_sasr_snapshot()`) — closed by revoking `INSERT` from both roles on both tables, matching the discipline this decision already applies to `organization_affiliations`. And `organization_affiliations` itself, despite the DML revoke this decision made, had no trigger stopping the **owner** connection from a raw `UPDATE`/`DELETE` — closed with a guard trigger reusing the `organizations` reparent guard's own GUC (see `docs/schema-design-2.md` §2f F49). None of this changes the principle; it finishes applying it. See `docs/schema-design-2.md` §2f (F47, F49, F50, F51) and DECISION-140.

**Correction (2026-09-24, tech-lead Phase 3 amendment after batch A, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`):** the function-mediation rule held up under build, but as implemented it is undermined by two grants this decision did not anticipate. `organization_lifecycle_events` shipped append-only *to `presby_app`* (no UPDATE/DELETE grant, correctly), but `presby_platform` — the `getPlatformDb()` connection, which already bypasses RLS by design — was granted full `select, insert, update, delete` on both `organization_lifecycle_events` and `organization_affiliations`, when Phase 3's own Ruling 3 only ever required `insert` (for `createOrganization()`'s affiliation row). A `SECURITY DEFINER` function runs with its *owner's* privileges regardless of what its caller is granted, so `presby_transfer_affiliation()` needed none of that extra grant to do its job — the widening bought nothing and reopened exactly the "raw mutation bypasses the function's authority check, uniform-message discipline, and provenance rule" hole this decision exists to close, on the one connection that already has the most reach. Ruled: narrow `presby_platform` to `select, insert` on both tables, and add a `presby_freeze_lifecycle_event()` `BEFORE UPDATE OR DELETE` trigger on `organization_lifecycle_events` mirroring `roll_actions_freeze` — a trigger, not merely an absent grant, because (as `presby_guard_organizations_delete()` already establishes in the very same migration) `BYPASSRLS` exempts a role from RLS policies, never from triggers, and the owner connection is precisely the path the 2026-08-31 58-org cascade went through. Required before batch B proceeds; not deferred.

**Second correction (2026-09-24, tech-lead Phase 3 amendment after batches B and C, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`):** two updates confirming this decision's mechanism as built. First, the grant-narrowing above binds `presby_app`/`presby_platform` as *grant subjects*, but `getPlatformDb()` itself connects as `neondb_owner`, which holds every privilege by ownership regardless of grant — so the function-mediation rule's real enforcement on that connection was always the trigger, never the grant (`docs/schema-design-2.md` §2d F44; same fact recorded against DECISION-136 above). Second, the recipient-side read this decision promised — `presby_list_published_returns_to_me()` — shipped filtering on `recipient_org_id = presby_current_org() and withdrawn_at is null`, resolving a contradiction between this decision's own text and the Phase 3 API Contract's prose (which had said the base query does *not* filter on `withdrawn_at`) in favor of the design authority: the filter applies, and the withdrawal columns stay in the return signature so surfacing a withdrawn row later is a `WHERE`-clause change, not a signature change. Withdrawal itself is now built as a set-once transition on `publications.withdrawn_at`/`withdrawn_by`/`withdrawn_minute_reference` together, enforced by `publications_freeze`, written only by a future `presby_withdraw_publication()` — confirmed as this decision's writer, in exactly `presby_transfer_affiliation()`'s confused-deputy shape, not a new mechanism. `docs/schema-design-2.md`'s D20 table row is updated to list the two new columns.

**Third correction (2026-09-24, tech-lead Phase 3 amendment after Phase 5 Finding 1, `docs/work-log/2026-09-24-lifecycle-affiliation-returns.md`):** this decision's function-mediation rule was not applied to `organization_successions`, the third of the three-axis tables, and QA demonstrated the gap with a reproducible cross-council write: a tenant with no read access to a lifecycle event (RLS-filtered to 0 rows) could still `INSERT` and `DELETE` succession rows naming that event's id, because the table carried `presby_app` grants of `SELECT, INSERT, DELETE`, no RLS, and no `organization_id`. Phase 3's Data Model justification for the exception — "visibility follows the parent `event_id`'s tenant policy via a join" — was false for writes; a join constrains what a SELECT returns, not what an INSERT is permitted to name. Fixed in `drizzle/0044` (not a new migration): `presby_app` narrowed to `SELECT` only, `presby_platform` to `select, insert`, and a `SECURITY DEFINER` `BEFORE INSERT` trigger asserts the named `event_id` exists and, when a tenant context is set, belongs to `presby_current_org()` — the same function-mediation shape this decision already gives `organization_affiliations`, with the future `presby_record_lifecycle_event()` DEFINER function as the table's sole tenant-side writer. No `organization_id` column was added to the table: unlike its two siblings, `organization_successions` has no recording party distinct from the event's own subject, so a second copy of the event's `organization_id` would be a driftable duplicate, not a new fact. `docs/schema-design-2.md` §3 and §2d record the corrected shape.

---

**DECISION-134: A fifth tripwire, `npm run check:secrets` (`scripts/check-secrets-pii.mjs`), scans every `git ls-files`-tracked file for secrets and PII, added the same day the repo went public. It splits findings into HARD (private keys, cloud API-key shapes, JWTs — no annotation exists, ever) and SOFT (env-var-shaped secret assignments, off-allowlist emails, phone/SSN-shaped numbers — exempt a genuine false positive with `// leak-ok: <reason>`), matching `check-sql-date.mjs`'s own annotation convention rather than inventing a new one. The audit that motivated this found exactly one real, pre-existing violation on the current tracked tree: a generated dev password narrated into a 2026-07-01 work-log's prose, and a real congregation's real address/phone entered into `docs/STATE.md` while testing the org-profile feature — both redacted in the same pass, history left unrewritten per the operator's explicit instruction.** (2026-08-30, self-implemented, no work-log — Trivial-adjacent tooling/security infrastructure per the Classification table's own reasoning for the four prior tripwires, none of which have a dedicated work-log either.)

Three design calls worth recording. **First, HARD vs. SOFT is a real split, not a formality**: a private key or a cloud provider's API-key shape should never exist in this repo under any circumstance, so giving it an escape hatch would be a footgun — SOFT findings need the escape hatch precisely because email/phone/generic-secret-shaped patterns have a real false-positive rate (git SSH remotes, `.env.example`'s own template connection strings, this project's own well-documented `e2e-fixture-only-not-a-secret`-style self-labeling convention). **Second, the placeholder-recognition list is deliberately generous and named as extend-the-list-not-annotate-every-occurrence**: `fixture`, `do-not-use`, `not-a-secret`, `example`, and RFC 6238's own well-known TOTP test vector are all pre-recognized, because annotating five already-self-documenting fixture values one at a time teaches the next contributor to reach for the escape hatch reflexively instead of naming their fixture values clearly — the pattern this repo already used, rewarded rather than punished. **Third, this tripwire only ever sees the current tracked tree, never git history** — stated explicitly in the script's own header and the `/pre-push` skill's new Step 3e, so a future reader doesn't mistake a clean run for proof that nothing sensitive was ever committed; the one real STATE.md finding this pass caught was already several weeks old.

**Status:** Resolved · **Date:** 2026-08-30

---

**DECISION-133: The staff/officer filter splits department/office matching into TypeScript (post-SQL) while kind/hasPriority stay SQL parameters — a forced, not stylistic, split, because office LABELS live in exactly one place (`OFFICE_LABELS`, DECISION-131) and the filter must match either the raw enum or its label; the committee `SECURITY DEFINER` function returns a flat, `group_name`-tagged list (never `group_id`) so one function/one call shape serves both a per-committee page and an all-committees page; `PersonCard` ships with no `phone`/`email` props at all rather than `StaffPerson`'s optional-and-caller-disciplined shape; `publicDisplayOrder` gets its own single-purpose `setXPublicDisplayOrder()` mutation per table rather than an optional field folded into the existing `setXPublicListed()` toggle, and is NOT Rule-7-audited (a presentation fact, not a disclosure fact); no `sort` filter key exists — priority ordering falls out of the roster's own unconditional `ORDER BY` for free.** (2026-08-28, tech-lead Phase 3, `docs/work-log/2026-08-28-public-directory-primitives.md`, extending DECISION-132's Phase 2 rulings.)

**First, the filter-matching split.** `kind` and `hasPriority` are SQL parameters on the widened `presby_public_staff_roster()` — exact predicates on real columns, and `hasPriority` must interact with `ORDER BY` to be useful at all. `department`/`office` are matched in TypeScript, inside `getPublicStaffRoster()`, on the SQL layer's own output — forced by DECISION-131's ruling that office display labels live only in `OFFICE_LABELS`, never duplicated as a SQL `CASE`: an `office` filter has to match either the raw enum (`clerk_of_session`) or its label (`Clerk of Session`), and doing that in SQL would mean either reopening DECISION-131 or restricting authors to the uglier raw spelling. Once `office` has to live in TS, `department` moves there too for symmetry (one normalization helper, one call site) rather than splitting free-text matching across two layers by field. Filtering after the SQL's own `ORDER BY` doesn't disturb order — it only removes rows.

**Second, the committee function's shape.** `presby_public_committee_roster(slug, committee?, hasPriority?)` is a NEW sibling function (confirmed by Phase 2: committees are a structurally different read, never a widening of the staff/officer union) returning a flat list where every row carries its own `group_name` (text) — never `group_id`. This single shape serves both cases Phase 1 asked about: a per-committee page supplies `{"committee": "Missions Committee"}` and gets back one group's roster; an all-committees page supplies `{}` and gets back every currently-public committee's rows in one array, which the render component groups sequentially by `group_name` (the SQL's own `ORDER BY group_name, ...` already clusters them contiguously — no re-sort needed). `groups.name` carries no org-level uniqueness constraint (an index, not a unique key) — two same-named committees would merge under one heading on an all-committees page; ruled an accepted admin-data-quality edge case, not a defect, since no `id` is ever exposed either way and it's no more revealing than two staff rows sharing a `department` string.

**Third, `PersonCard`'s props.** Rather than carry `StaffPerson`'s optional `phone`/`email` fields onto the new component and rely on live callers' own discipline not to set them (the footgun Phase 2 Note 4 flagged), `PersonCard` simply has no such props — a structural fix instead of a documented caution, since it's a new component with no hand-authored-block use in this pass. No shared internal fragment renderer with `StaffList` was extracted for the ~6 overlapping lines of markup (photo/name/optional-title) — the two components' container contracts (`<li>`-in-`<ul>` vs. standalone `<div>`) and prop shapes diverge enough that a shared helper would need more branching than it saves, and would recouple two components whose future evolution paths (hand-authored content vs. live, consent-gated data) are meant to stay independent.

**Fourth, `publicDisplayOrder`'s own mutation shape.** A dedicated `setStaffPositionPublicDisplayOrder()`/`setOfficerTermPublicDisplayOrder()`/`setGroupMembershipPublicDisplayOrder()` per table, not an optional field folded into the existing `setXPublicListed()` toggle's input — every `set*`/`start*`/`end*` function in this codebase already does exactly one thing, and folding the two together would introduce a real "is an omitted key 'leave unchanged' or 'clear it'" ambiguity across a `FormData` boundary that a required (non-optional) `publicDisplayOrder: number | null` input on its own mutation never has to resolve. Ruled NOT Rule-7-audited: unlike the toggle itself (a disclosure fact — DECISION-131), reordering an already-public person's position in a roster changes nothing about who is visible, the same "content configuration" reasoning DECISION-113 already used to exempt `events.manage`.

**Fifth, the removed `sort` key.** Phase 1 speculated a marker shape of `{"filter": {"hasPriority": true}, "sort": "priority"}`; ruled unnecessary and removed — both `SECURITY DEFINER` functions' `ORDER BY` is unconditionally `coalesce(public_display_order, 2147483647), display_name`, so `hasPriority: true` alone already narrows to and orders the curated subset. A separate `sort` key would only ever describe an invalid/incoherent state (priority-ordering a set where priority is mostly unset) — removing the key removes that state rather than validating against it.

**Sixth, migration numbering:** confirmed `drizzle/0042_presby_public_directory_primitives.sql` as the next free number by direct `ls`/`_journal.json` check, with no concurrent `docs/TODO.md` claim found — following the 0039/0040/0031 near-collision precedent's own "re-check immediately before applying" discipline.

**Status:** Resolved · **Date:** 2026-08-28 · **Feature:** `2026-08-28-public-directory-primitives` (Phase 3)

---

**DECISION-132: The `liveSlots` mechanism becomes resolver functions (`(filter) => ReactElement | null`), not a bundle-introspection pre-walk — rejecting Phase 1's own named recommendation in favor of the alternative it raised — because it brings `liveSlot` in line with every other block renderer's `(props, ctx)` shape, eliminates a key-collision/traversal-order coupling the pre-walk design never actually resolved, and requires no new `presby-site-kit` export. Treated as a breaking change to `RenderSiteBundleInput.liveSlots`'s type, versioned accordingly. A `publicDisplayOrder` integer column (not an id-list-in-marker) is adopted on `staff_positions`/`officer_terms`/`group_memberships` for hand-curated ordering, keeping `id` out of the anonymous `SECURITY DEFINER` projection permanently. A separate `sites.public_committee_directory` flag (not folded into `sites.public_staff_directory`) gates the committee union independently.** (2026-08-28, architect Phase 2, `docs/work-log/2026-08-28-public-directory-primitives.md`.) `mdxAst` is already a flat `ContentBlock[]` array before any renderer sees it, so a bundle-introspection pass would just be a second traversal of the same array `renderSiteBundle()` already iterates — and Phase 1's own pre-walk design never named a key scheme letting two same-named markers with different filters coexist in one `Record<string, ReactElement>` (a filter-derived key requires the marker to independently reproduce it; an occurrence-index key requires two separate traversal orders to match forever). Resolver functions resolve each marker exactly where it's encountered, using exactly that marker's own props, verifiably RSC-safe since `renderSiteBundle()` runs synchronously inside `page.tsx`'s own server-component render with no client boundary crossed. The no-runtime-queryable-filter redline is enforced by two concrete review checks: no `searchParams` between `(public)/site/[slug]` and the public-read call sites, and every `liveSlots` resolver closure in `page.tsx` may only close over `slug`/URL-derived values, never `request`/`headers()`/`cookies()`.

**Status:** Resolved · **Date:** 2026-08-28 · **Feature:** `2026-08-28-public-directory-primitives` (Phase 2)

---

**DECISION-131: The public staff-directory's `SECURITY DEFINER` roster function returns raw `kind`/`role_raw` and defers office-label mapping to TypeScript (`OFFICE_LABELS`), rather than duplicating a label `CASE` in SQL; the on/off toggle's `public_listed_by`/`public_listed_at` are rewritten on every call in either direction (unlike `recordedBy`'s set-once-at-creation precedent); and v1 ships with `people.photo_key` upload deliberately unwired, not folded into this pipeline.** (2026-08-27, tech-lead Phase 3, `docs/work-log/2026-08-27-public-staff-directory.md`.) Turns Phase 2's architectural rulings (generic `liveSlots`, a new `SECURITY DEFINER` function, `recordAudit()` required) into a buildable shape. **First:** `presby_public_staff_roster(text)` returns `officer_terms.office`'s raw enum value plus a `kind` discriminator, not a pre-labeled string — `OFFICE_LABELS` (`src/lib/officers.ts`) stays the one place office display labels are spelled out; a SQL-side `CASE` would create a second copy that silently drifts the next time an office is added. **Second:** both `setStaffPositionPublicListed()`/`setOfficerTermPublicListed()` update `public_listed_by`/`public_listed_at` on every call, turning the bit off included — a deliberate departure from `recordedBy`'s "attribute the row's creation, never touch it again" pattern, because *this* fact is "who most recently changed visibility," which is exactly as true, and exactly as worth surfacing on the admin roster row, in the off direction as the on one. **Third:** photo upload for `people.photo_key` is named and deferred rather than silently absent — the serving mechanism (the existing `(public)/site/[slug]/assets/[key]/route.ts`, which resolves any blob key by `organizationId` already) needs no new code to pick up a photo once one exists, so deferring costs nothing but an upload UI that would have otherwise widened Phase 4's blast radius across an already four-step, two-repo implementation sequence.

**Status:** Resolved · **Date:** 2026-08-27 · **Feature:** `2026-08-27-public-staff-directory` (Phase 3)

---

**DECISION-130: Org feature categories' resolver/CRUD module (`src/lib/org-feature-categories.ts`) has a strictly one-directional dependency on `src/lib/org-features.ts` (never the reverse), which forces `toggleOrgFeatureCategory()` to skip `recordAudit()` entirely and pushes the per-`feature_key` audit enumeration into the Server Action instead; `organization_feature_categories.category` gets a schema-layer CHECK constraint, departing from `organization_feature_toggles.feature_key`'s deliberately open, resolver-validated-only precedent; the new `org_portal.feature_categories` flag is a dual-purpose UI-section gate and axis kill-switch, not merely a page-visibility toggle.** (2026-08-27, tech-lead Phase 3, `docs/work-log/2026-08-27-feature-categories.md`.) Three implementation calls turning the architect's Phase 2 re-run rulings (default-on resolution; a new, separate resolver module) into a buildable shape. **First, the audit split:** `org-features.ts`'s `isOrgFeatureEnabled()` must import the new module's `categoryEnabledInTx()` to compose the two axes inside one transaction — a real, load-bearing one-way dependency, not a style preference. Since `org-feature-categories.ts` therefore cannot also import `ORG_FEATURE_CATALOG` back from `org-features.ts` without creating an actual import cycle, its `toggleOrgFeatureCategory()` cannot itself enumerate which `feature_key`s a category mutation affects — the one piece of audit granularity the architect's conditional approval of reusing `org_features.manage` required. Resolution: `toggleOrgFeatureCategory()` follows the check-then-write-only shape (no `recordAudit()` call inside it), and the Server Action (`admin/features/actions.ts`, which can import both modules with no cycle) computes `affectedFeatureKeys` and calls `recordAudit()` itself — mirroring `role-grants.ts`/`admin/roles/actions.ts`'s lib-does-check-and-write/action-does-audit split rather than `org-features.ts`'s own audit-in-lib split for `toggleOrgFeature()`, and named here as a deliberate, forced divergence from the sibling file's pattern, not an inconsistency. **Second, the CHECK constraint:** `organization_feature_categories.category` gets a schema-layer `check (category in (...))` against the closed six-value `PortalDomain`-minus-`administration` set, unlike `organization_feature_toggles.feature_key`, which is deliberately unconstrained at the schema layer (validated only at the resolver, so it can mirror `feature_flags.key` strings by convention without a table owning the other). `category` has no such open-catalog reason to stay unconstrained — it is a fixed, six-value business taxonomy, and Phase 1 Gap 2 (`"administration"` must never become a selectable category) is worth a defense-in-depth database constraint on top of the resolver-layer `isCategoryKey()` guard. **Third, the flag's scope:** `org_portal.feature_categories` is built to gate two things, not one — the category-picker UI section's visibility (matching every prior `org_portal.*` addition's "ships dark until the page lands" convention) AND whether `isOrgFeatureEnabled()`'s composition consults the category table at all. When off, the axis is fully inert (every category resolves enabled, no DB read even fires), making the flag a genuine rollback lever for the whole mechanism, not just a hidden-UI convenience — the same reasoning DECISION-124 gave `platform.merged_home` for being a content-level kill switch rather than a routing-level one.

---

**DECISION-129: `staff.manage` (tier 1) binds to NO existing office — DECISION-078's test is run individually against every constitutional office/role in the catalog and fails all of them — so a new, org-type-neutral template role `personnel_admin` is minted; `createPerson()`'s `rollAction: { kind: "none" }` variant (DECISION-128) sets `memberships.engagementStatus = "staff"`, not the branch's existing hardcoded `"regular"` — the load-bearing fix that keeps a staff-only anchor row out of `getDirectory()`'s and `findPersonMatches()`'s existing `engagement_status = 'regular'` eligibility branch; staff hiring/termination mutations are ruled NOT Rule-7-audited, on a sharper basis than "personnel administration, not security" — `staff_positions` structurally cannot fan out into `group_memberships`/access the way `officer_terms` does, so it has no access-change nexus at all, not just a weaker one.** (2026-08-27, tech-lead Phase 3, `docs/work-log/2026-08-27-staff-and-personnel.md`.)

**Status:** Resolved · **Date:** 2026-08-27 · **Feature:** `2026-08-27-staff-and-personnel` (Phase 3)

Three implementation calls turning DECISION-128's Phase 2 ruling and Phase 1's design into a buildable shape. **First, the DECISION-078 test, actually run** (not assumed, per the architect's explicit instruction) against every role/office in the current catalog: `stated_clerk`/`presbytery_stated_clerk` (register-keeping duty, not personnel), `treasurer` (financial-officer duty; v1 carries no compensation data for this office's duty to attach to), `installed_pastor` (pastoral/teaching duty; supervising staff is common practice but not a named constitutional duty), `session_member`/Session-as-a-body (spiritual governance; approving a hire is a discretionary body action many congregations delegate, not a named individual office duty — same "administratively empowered, not constitutionally dutied" shape DECISION-115/119 already used to decline `stated_clerk` accretion), `diaconate_member` (mercy/visitation duty), `trustee` (the closest textual candidate — property/real-estate custodianship per its corporate-trustee function, not personnel administration across every department the new table must cover), `moderator` (presiding function only), `brand_admin`/`member_care_admin`/`support_contact`/`role_admin`/`children_ministry_admin` (each a single-purpose non-constitutional software role with no overlap), `committee_chair` (a generic per-committee template, not a personnel-committee-specific one), and `executive_presbyter` (named but never implemented — DECISION-112 already rejected it for `credentials.manage` as "program leadership, not register-keeping," and the same reasoning excludes it here). Every one fails — confirming Phase 1's own finding, now demonstrated rather than assumed. **No default role binding.**

**Second, the new role's shape.** Rather than repeat `member_care_admin`/`children_ministry_admin`'s fixture-only-per-org pattern, `staff.manage` needed a role that is genuinely org-type-neutral (Phase 1's own point 3 — congregations AND presbyteries both employ staff, with no polity asymmetry the way ordination/appointments have), so it uses the `committee_chair`/`presbytery_stated_clerk` template mechanism instead (`organization_id IS NULL, organization_type_scope IS NULL` — universal, unlike `presbytery_stated_clerk`'s presbytery-only scope): a new template role `personnel_admin` (constitutional, protected, carrying `staff.manage` alone — a single-purpose role, not a wildcard, matching every DECISION-078-minted role's own discipline), self-serve adoptable by any organization through the existing `/admin/roles/new` → `listTemplateRoles`/`adoptTemplate` flow (DECISION-109), with no bootstrap gap the way `stated_clerk`/`officers.manage` still carry. `scripts/seed-dev.sql` adopts the template into two fixture-only per-org rows (one congregation, one presbytery) for dev reachability, each granted to a person holding no other role today, matching DECISION-109's own "don't stack a third capability onto an already-multi-role fixture person" discipline.

**Third, and the actual defect this design catches before it ships:** `createPerson()` (`src/lib/people.ts`) hardcodes `memberships.engagementStatus: "regular"` on every call today, unconditionally. `getDirectory()`'s own eligibility formula (`src/lib/directory.ts`) and its copy in `findPersonMatches()` (`src/lib/org-portal/find-person.ts`) both read `current_roll IN (...) OR engagement_status = 'regular'` — so a staff-only anchor row created via the new `rollAction: { kind: "none" }` variant, if it kept the existing hardcoded value, would satisfy that OR-branch on `engagement_status = 'regular'` alone (since `current_roll` stays null by design) and leak straight into the public congregational Directory and the `admin/members` roster that reuses it — the exact Architect Note 3 gap, but concretely reachable the moment staff hiring's inline create ships, not a hypothetical. Resolution: the `rollAction.kind === "none"` branch sets `engagementStatus: "staff"` instead of `"regular"`; `"regular"`/`"visitor"` stay unchanged for every other `rollAction` kind. Because both eligibility predicates check for the literal string `'regular'`, this one-line branch fix closes the leak with zero changes to `directory.ts`/`find-person.ts` — confirmed by reading both formulas directly rather than assuming the new value would be excluded. `admin/members`'s own list needs no separate filtering fix (Phase 1 Gap 3/Architect Note 3 resolved as a non-issue, contingent on this exact detail landing in Phase 4, not as a general assertion).

**Fourth, audit scope**, sharper than the architect's own flagged analogy: `officer_terms` mutations ARE Rule-7-audited (`OFFICER_TERM_STARTED`/`OFFICER_TERM_ENDED`) despite also being "personnel-administration"-shaped, specifically because `officer_terms_sync_derived` (trigger) propagates a term's start/end into `group_memberships` — a real, structural access change (Session/Diaconate group membership, and everything role-resolution derives from it). `staff_positions` carries no such trigger and no FK into `role_grants`/`group_memberships` at all (Phase 1's own "grants nothing by itself" finding, confirmed unchanged by this design) — so the analogy to `events.manage`/`organization_service_times` (DECISION-113, content configuration, no access nexus) is the correct one, not the officer_terms analogy the surface-level "personnel administration" label might suggest. Staff hiring/termination mutations are NOT audited; `admin/staff/actions.ts` carries a `// audit-exempt:` comment on each mutation naming this reasoning for the next reader, even though the mechanical `check:audit` tripwire (which only regexes literal `db.insert`/`update`/`delete` inside `actions.ts` files) will not fire either way, since the actual writes live in `src/lib/staff.ts` behind a `tx.insert`/`tx.update` call, matching `officers.ts`'s own indirection.

---

**DECISION-128: Staff/personnel gets a new `src/lib/db/domain/staff.ts` module (not folded into `officers.ts` — deliberately orthogonal to the ecclesiastical register, not a fourth "who serves in what capacity" shape there); DECISION-116's "no inline person-create, block and redirect to Members" ruling is not amended, but `createPerson()` gains a third `rollAction: { kind: "none" }` variant so staff hiring can call the same shared, F21-safe person-creation core inline without fabricating a roll event.** (2026-08-27, architect Phase 2, `docs/work-log/2026-08-27-staff-and-personnel.md`.) Staff positions are forced by F2 to composite-FK through `memberships`, the same as every other "who serves" table — that part of DECISION-116's discipline (a CURRENT membership row required first, matched only through `presby_match_person()`, no bespoke duplicate-validation person-creation surface) is not in tension with staff and is not reopened. What breaks under unmodified DECISION-116 is the *destination* it points to: `createPerson()` (`src/lib/people.ts`) requires a `rollAction` and unconditionally writes a `roll_actions` row, because for DECISION-116's own case (a transferring-in minister) receiving membership and being received onto the constitutional roll are the same G-2.0402 event. For staff they are not — Phase 1's design is explicit that `staff_positions` must never imply a roll status, and forcing an outside bookkeeper through `profession_of_faith`/`other_participant_enrolled` to obtain an anchor row would fabricate a roll fact, a "Roll Is the System of Record" violation, not merely clumsy UX. Resolution: extend `createPerson()`'s `rollAction` union with `{ kind: "none" }`, skipping step 4 (no `roll_actions` insert) while keeping steps 1–3 (person / household / membership) unchanged; staff hiring's inline "add a new person" affordance is a thin caller of this same function, never a second implementation. Permission gating splits accordingly: `people.manage` required unconditionally for the create-new-person branch, `roll.propose` required only when `rollAction.kind !== "none"`; `staff.manage` alone is sufficient to attach a position to anyone already matched via `presby_match_person()`, but not to create a brand-new `people` row (that stays gated on `people.manage` — a personnel-module permission must not silently compose into general people-creation power, per "No Role Carries a Wildcard"). Exact signature/gating implementation is Phase 3's to finalize.

**Status:** Resolved · **Date:** 2026-08-27 · **Feature:** `2026-08-27-staff-and-personnel` (Phase 2)

---

**DECISION-127: The operator-supplied PresbyPortal Brand Guidelines v1.0 kit (navy `#0D2240` / blue `#1A5AA8` / gold `#F2B134`, Inter/Inter Display, a real logo) becomes the platform's default palette and identity, replacing the placeholder palette DECISION-045/slice-0 had only brought up to the contrast floors — closing G1's own long-standing note in `src/lib/brand/contract.ts` that "the default is deliberately NOT presby's marketing identity ... a separate visual decision."** (2026-08-27, `docs/work-log/2026-08-27-presbyportal-brand-kit.md`.) Scoped to platform chrome and the marketing home page only — per-org branding (`organization_brands`, the ramp generator's derivation logic) is untouched; a congregation's own brand still overrides exactly the tokens DECISION-046's cascade already permits. Two deliberate deviations from the kit's literal deliverable: (1) `--destructive` uses `#B22B1E`, not the kit's `#B4232A` — the ramp generator's exhaustive 288-seed property test (D6, OKLCH hue distance ≥45°) caught the kit's exact hex sitting 0.1° inside the floor against one red-violet congregation seed; nudged back toward the prior palette's pure-red hue, which that suite was already proven safe against, same visual family and contrast floor; (2) dark mode is an original derivation, not part of the kit (which ships one scheme), following the app's existing light→dark derivation pattern and independently re-verified against every D1–D4 pair. `--radius` (DECISION-048) is untouched — the kit's own radius scale belongs to a different product surface, not this app's load-bearing corner-radius decision.

**Status:** Resolved · **Date:** 2026-08-27 · **Feature:** `2026-08-27-presbyportal-brand-kit`
