...
/**
 * Tests for <DisplayOrderInput> (staff) — public-directory-primitives
...
vi.mock("./actions", () => ({
  setStaffPositionPublicDisplayOrderAction: (...args: unknown[]) =>
    mockAction(...args),
}));
...
describe("DisplayOrderInput (staff) — initial render", () => {
...
describe("DisplayOrderInput (staff) — no-op blur", () => {
...
describe("DisplayOrderInput (staff) — committing a change", () => {
  it("commits the parsed integer on blur, and refreshes the router on success", async () => {
    mockAction.mockResolvedValueOnce({
      ok: true,
      data: { positionId: "position-1", publicDisplayOrder: 2 },
    });
    renderInput(null);
    const input = screen.getByLabelText(/Marisol Windham/i);
  ...
  it("commits null when the field is emptied out", async () => {
    mockAction.mockResolvedValueOnce({
      ok: true,
      data: { positionId: "position-1", publicDisplayOrder: null },
    });
    renderInput(2);
    const input = screen.getByLabelText(/Marisol Windham/i);
  ...
  it("commits on Enter the same way it commits on blur", async () => {
    mockAction.mockResolvedValueOnce({
      ok: true,
      data: { positionId: "position-1", publicDisplayOrder: 5 },
    });
    renderInput(null);
    const input = screen.getByLabelText(/Marisol Windham/i);
    input.focus();
...
describe("DisplayOrderInput (staff) — client-side validation", () => {
...
describe("DisplayOrderInput (staff) — denied server result", () => {
