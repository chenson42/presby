...
knows what exists without re-reconning. **Version `0.12.0` · surveyed 2026-08-21.**
...
- **presby: org portal** — `/o/<slug>` tenant-facing pages. **Portal home v2** (gated `org_portal.home_v2`, seeded off): time-aware greeting, find-a-person (server action, single-match jump, `directory.view` re-checked in the action), "Yours" household zone (renders only when non-empty), flag-mirroring tool-tile registry (`src/lib/org-portal/tiles.ts`); flag off → the original stub. **Directory v2** (gated `org_portal.directory` + `org_portal.directory_v2`, seeded off): searchable card grid (name/email/phone ILIKE, shared privacy predicate), households view with zero-visible-member drop, person detail (`/directory/[personId]`) and household detail (`/directory/households/[householdId]`) pages, per-household DeaconCard derived from `officer_terms.org_unit_id` (migration 0025 — a deacon's term carries its district; vacant districts render a neutral state), parishes deacon-roster view + privacy-hidden-row visibility with lock badges for `directory.view_hidden` holders (row-level only; field-level hides stay applied — Phase 6 ratified); flag off → the original flat list (`src/lib/directory.ts`, `src/app/(org)/o/[slug]/directory/`, `docs/work-log/2026-08-24-portal-home-directory.md`). **Portal chrome** (gated `org_portal.chrome_v2`, seeded off): the org's own logo/OrgMark replaces the platform wordmark in the header inside `/o/<slug>` (linking to the org home; OrgSwitcher stays the sole name-bearing element) plus a persistent, wrapping, flag-filtered menu row (`portal-nav.tsx`, active-state client leaf; feedback entry on its own `org_portal.feedback` flag; `docs/work-log/2026-08-25-portal-chrome.md`). Also: role administration (`/o/<slug>/admin/roles`, `role_grants.manage` permission, gated `org_portal.roles`) — grant/revoke/view *existing* `role_grants` rows against *existing* `app_roles`, creates neither — and support tickets (`/o/<slug>/tickets*`, `/o/<slug>/feedback`, `tickets.file` permission, gated `org_portal.tickets`). All three flags seeded off. First tenant-facing capabilities a congregation's own staff — not a platform admin — can act on. `src/lib/directory.ts`, `src/lib/role-grants.ts`, `src/lib/tickets.ts`, `src/app/(org)/o/[slug]/{directory,admin/roles,tickets,feedback}/`.
