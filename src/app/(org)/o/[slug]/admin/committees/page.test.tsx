...
/**
 * Orchestration tests for `/o/<slug>/admin/committees`'s page.tsx —
 * product-IA scaffold, docs/work-log/2026-08-27-product-ia-scaffold.md
 * (Phase 3 §3/§9, DECISION-117). Mirrors `admin/credentials/page.test.tsx`'s
 * flag-before-org-type ordering contract. `committees` is a
 * presbytery-scoped placeholder tile.
 */
...
import CommitteesPage from "./page";
...
describe("CommitteesPage — the shared four-way miss response", () => {
  it("redirects to /signin with a callbackUrl back to admin/committees when unauthenticated", async () => {
    cachedAuth.mockResolvedValue(null);

    await expect(CommitteesPage({ params: makeParams() })).rejects.toThrow(
      "REDIRECT:/signin?callbackUrl=%2Fo%2Fnorthern-reach%2Fadmin%2Fcommittees",
    );
  ...
  it("renders the shared access-denied copy for a forbidden org relationship", async () => {
    ...
    const el = await CommitteesPage({ params: makeParams("other-presbytery") });
    render(el);
  ...
  it("renders the shared ended-relationship copy for an ended relationship", async () => {
    ...
    const el = await CommitteesPage({ params: makeParams("fernwood") });
    render(el);
...
describe("CommitteesPage — the flag-before-org-type ordering contract", () => {
  it("calls assertOrgAccess before checking the flag", async () => {
    cachedAuth.mockResolvedValue({ user: { id: "u1" } });
    resolveOrgContext.mockResolvedValue(OK_RESOLVED_PRESBYTERY);
    isFlagEnabled.mockResolvedValue(false);

    await CommitteesPage({ params: makeParams() });

    expect(assertOrgAccess).toHaveBeenCalledWith("person-1", "org-1");
  });

  it("flag off renders PlaceholderFlagOff, checked with org_portal.committees", async () => {
    cachedAuth.mockResolvedValue({ user: { id: "u1" } });
    resolveOrgContext.mockResolvedValue(OK_RESOLVED_PRESBYTERY);
    isFlagEnabled.mockResolvedValue(false);

    const el = await CommitteesPage({ params: makeParams() });
    render(el);

    expect(isFlagEnabled).toHaveBeenCalledWith("org_portal.committees");
    expect(
      screen.getByText(
  ...
  it("flag off wins over org type — a congregation with the flag off still sees PlaceholderFlagOff, not PlaceholderNotAvailable", async () => {
    ...
    const el = await CommitteesPage({ params: makeParams("alder-creek") });
    render(el);
  ...
  it("flag on + correct org type (presbytery) renders ComingSoon", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Committees & Commissions" }),
    ).toBeTruthy();
    expect(
