...
- **`getPlatformDb()`**, which bypasses RLS entirely. (The `presby_app` grant shape backing this boundary is written down and enforced in `drizzle/0048_presby_security_b.sql` rather than living only in undocumented live grants — DECISION-146, 2026-09-25.) It exists only for platform-admin pages that legitimately need to see across every tenant (user management, cross-org ticket triage). `users.is_platform_admin` decides which *pages* are reachable — it never decides whether a query is filtered. Conflating those two questions is exactly the bug class this split exists to prevent.
