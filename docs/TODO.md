...
- [ ] **Operator ruling: the dark `--destructive` correction.** Slice 0 shipped the design's values (fill `hsl(0 84% 48%)` + white), which fixes a token pair nothing currently renders and drops rendered `text-destructive` on the dark page from 4.73 to 3.67. Phase 1's Flow 3 rule ("move the text, not the hue") gives a strictly better two-line alternative: keep `hsl(0 84% 60%)` and set the dark `--destructive-foreground` to the page ink `hsl(222 47% 11%)` — 4.73 on the pair, 4.73/3.92 retained as text — `docs/work-log/2026-08-19-brand-foundation.md` Phase 4 commit `0.1`
