...
- [ ] **`/developer` is blind to half the schema.** Table/column structure comes from `getTableConfig()` at request time and descriptions from `pg_description` live, so both auto-update — but Drizzle knows nothing about functions, triggers, or RLS policies, and the page never queries `pg_proc` / `pg_trigger` / `pg_policies`. For presby that is the half carrying the invariants: 13 `presby_*` functions, every FORCE RLS policy, the roll-freeze and derived-roster triggers. The invariants list is a hand-maintained array in `src/lib/dev-docs.ts:320` and drifts like any prose. Read them live in the same pattern `loadDescriptions()` already uses — `pg_proc.prosecdef` would *show* which functions are SECURITY DEFINER, exactly the F26 property — and derive most of the invariants list instead of maintaining it. **P0 makes this concrete**: it renames a function and adds two triggers, none of which will appear on the page. Also: `docs/schema-design.md`'s ER diagrams are a committed copy needing `npm run docs:erd`, unlike `/developer/erd` which is live — put it in the ship-time housekeeping cluster (Rule 14) — found 2026-08-18
