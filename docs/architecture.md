...
4. **Authority flows up by publication, never down by inheritance.** A presbytery sees what a congregation is required to publish to it and nothing else by default. This is no longer just a stated principle — it's a real mechanism now: a congregation's own clerk publishes an annual statistical snapshot, and the write that creates the presbytery's copy of it is a single, narrow database function that derives its own target (the calling congregation's actual parent presbytery) rather than trusting anything the caller supplies — the platform's first write that ever crosses a tenant boundary, and it was built to be the only kind that's allowed to. The only other exceptions — an administrative commission, and a session-granted delegation — are both explicit, time-boxed, and minuted, not ambient access.
   ...
   Since September 2026 this rests on a three-axis model of an organization rather than a single parent pointer. *Existence* is an immutable, minuted event history — a congregation is organized, received, merged, divided, dismissed or dissolved, in the Book of Order's own words, and never deleted. *Council affiliation* — which presbytery a congregation belongs to, and since when — is its own dated history, because a congregation's presbytery can change by redistricting without the congregation changing; the cached parent pointer is derived from that history and cannot be written directly. *Platform participation* (is this organization a tenant?) stays a separate column. Publication is then an immutable event that references an immutable artifact — the congregation's frozen statistical return — and records who published it, to whom, under what session minute, and whether it was later superseded or withdrawn. A presbytery reads the artifact that was published to it, as of the affiliation in force at the time; it never reads the congregation's live records, and a later redistricting cannot retroactively change who received an already-filed return. Every write or read that crosses a council boundary goes through one narrow database function that derives the acting council from the session and its standing from the affiliation history, so no platform role acquires ecclesiastical authority merely by its position on the platform.
...
- **`getPlatformDb()`**, which bypasses RLS entirely. (The `presby_app` grant shape backing this boundary is written down and enforced in `drizzle/0048_presby_security_b.sql` rather than living only in undocumented live grants — DECISION-146, 2026-09-25 — and CI proves the whole schema is reproducible from `drizzle/` alone by building a database from empty on every run — DECISION-150, 2026-09-26.) It exists only for platform-admin pages that legitimately need to see across every tenant (user management, cross-org ticket triage). `users.is_platform_admin` decides which *pages* are reachable — it never decides whether a query is filtered. Conflating those two questions is exactly the bug class this split exists to prevent.
