...
erDiagram
  ...
  organizations ||--o{ organization_identifiers : "organization_id"
...
erDiagram
  ...
  users |o--o{ roll_actions : "proposed_by"
...
erDiagram
  ...
  users |o--o{ officer_terms : "recorded_by"
  users |o--o{ officer_terms : "public_listed_by"
  memberships ||--o{ officer_terms : "person_id"
  org_units |o--o{ officer_terms : "org_unit_id"
...
erDiagram
  ...
  users |o--o{ group_memberships : "public_listed_by"
...
erDiagram
  ...
  groups |o--o{ org_delegations : "group_id"
...
```mermaid
erDiagram
  organizations ||--o{ publications : "organization_id"
  organizations ||--o{ publications : "recipient_org_id"
  users |o--o{ publications : "authorized_by"
  users |o--o{ publications : "withdrawn_by"
  statistical_returns ||--o{ publications : "artifact_id"
  organizations ||--o{ statistical_returns : "organization_id"
  organizations ||--o{ statistical_returns : "about_org_id"
  sasr_form_versions ||--o{ statistical_returns : "form_version_key"
```

### K. Lifecycle and affiliation

```mermaid
erDiagram
  organizations ||--o{ organization_affiliations : "organization_id"
  organizations ||--o{ organization_affiliations : "subject_org_id"
  organizations ||--o{ organization_affiliations : "parent_org_id"
  users |o--o{ organization_affiliations : "recorded_by"
  organizations |o--o{ organization_affiliations : "closed_by_org_id"
  users |o--o{ organization_affiliations : "closed_by"
  organizations ||--o{ organization_lifecycle_events : "organization_id"
  organizations ||--o{ organization_lifecycle_events : "subject_org_id"
  users ||--o{ organization_lifecycle_events : "recorded_by"
  organization_lifecycle_events ||--o{ organization_successions : "event_id"
  organizations ||--o{ organization_successions : "predecessor_org_id"
  organizations ||--o{ organization_successions : "successor_org_id"
```
...
| **F19** | Scenario | **Death does not terminate anything.** A `death` roll action sets the roll and `date_of_death`, but leaves `officer_terms`, `role_grants`, and `group_memberships` open. A deceased elder stays on session and keeps every permission indefinitely. Needs a trigger ending all three as of the effective date. | **Open** — corrected 2026-08-26. §8's trigger only propagates `officer_terms.ends_on` → `group_memberships` after a human manually sets `ends_on`; nothing fires automatically off an approved `death` (or `removed_by_session`, `renunciation_of_jurisdiction`, `certificate_dismissed`, `affiliate_ended`, `other_participant_removed`, `other_loss`) roll action. Traced across every `drizzle/*.sql` trigger during `docs/work-log/2026-08-26-member-roll-on-edit.md` Phase 2/3 — no `terminate_person_participation()`-shaped routine exists yet. Was inert only because no UI could record `death` at all; that pipeline scoped the terminating kinds out rather than build the trigger as a co-requisite. |
